      ...
      <div className="screen-stack">
        ...
        {isQuestion ? (
          <Card className="answer-card">
            ...
            <div className="answer-options" role="radiogroup" aria-label="답변 선택">
            ...
            {recommendationShown && <InlineNotice tone="blue" title="AI 추천">B안은 권한이 단순하고 통제가 쉬워 현재 evidence와 가장 잘 맞습니다. 추천은 제안이며 인간 결정을 대체하지 않습니다.</InlineNotice>}
            {saved && <InlineNotice tone="mint" title="저장됨">{selectedOption?.title ?? "Human answer"}가 기록되었습니다. 관련 scope가 다시 계획됩니다.</InlineNotice>}
            <div className="button-row">
              <Button variant="primary" onClick={saveAnswer} disabled={!answer || !isOpen}>답변 저장</Button>
              <Button variant="neutral" onClick={() => { setAnswer("B"); setRecommendationShown(true); }}>AI 추천 보기</Button>
            </div>
          </Card>
        ) : (
          <Card className="answer-card">
