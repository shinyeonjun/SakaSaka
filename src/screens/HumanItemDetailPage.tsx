...
export function HumanItemDetailPage({ projectId, itemId }: { projectId: string; itemId: string }) {
  ...
  if (!project || !item) return <div className="screen"><Card className="empty-state"><h1>사람의 확인 항목을 찾을 수 없습니다.</h1><Button variant="neutral" onClick={back}>뒤로 가기</Button></Card></div>;
  ...
  return (
    <div className="screen">
      <PageHeading title={`${isQuestion ? "질문" : humanLabel(item.kind)} #${item.id}`} description="AI가 왜 사람에게 왔는지, 답변 전 무엇을 하고 있는지까지 보여줍니다." actions={<Pill tone={humanTone(item.kind)}>{isOpen ? (item.status === "DEFERRED" ? "나중에 답변" : isQuestion ? "답변 필요" : "검토 필요") : humanStatusLabel(item.status)}</Pill>} />
      <div className="screen-stack">
        <Card className="detail-intro-card">
          <Pill tone={humanTone(item.kind)}>{humanLabel(item.kind)} · {isQuestion ? "사람의 의도 필요" : "영향을 고려한 결정"}</Pill>
          <h2>{item.title}</h2>
        ...
        <div className="split-grid detail-context-grid">
          ...
          <Card>
            ...
            <p className="detail-strong">{item.blockingScope.length ? `보류됨 · ${item.blockingScope.join(" · ")}` : "보류됨 · 없음"}</p>
            <p className="detail-strong">{item.continuingScope.length ? `계속 진행 · ${item.continuingScope.join(" · ")}` : "계속 진행 · 관찰 대기"}</p>
        ...
        {isQuestion ? (
          <Card className="answer-card">
            ...
            {item.status === "DEFERRED" && <InlineNotice tone="yellow" title="나중에 답변 가능">이 질문은 닫히지 않았습니다. 준비되면 언제든 답변할 수 있습니다.</InlineNotice>}
            {saved && <InlineNotice tone="mint" title="저장됨">{selectedOption?.title ?? "사람의 답변"}가 기록되었습니다. 관련 범위가 다시 계획됩니다.</InlineNotice>}
            <div className="button-row">
              <Button variant="primary" onClick={saveAnswer} disabled={!answer.trim() || !isOpen}>답변 저장</Button>
              {item.status === "OPEN" && <Button variant="subtle" onClick={() => dispatch({ type: "RESOLVE_HUMAN_ITEM", itemId: item.id, action: "defer" })}>나중에</Button>}
            </div>
          </Card>
        ) : (
          <Card className="answer-card">
            <SectionHeader title={item.kind === "APPROVAL" ? "승인" : "사람의 응답"} />
            ...
            <div className="button-row">
              ...
              <Button variant="subtle" onClick={() => navigate(`/projects/${encodeURIComponent(projectId)}/needs-you`)}>도움 필요로 돌아가기</Button>
            </div>
          </Card>
        )}
      </div>
