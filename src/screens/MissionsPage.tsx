...
import { missionHistory, latestControlPlane, relativeAge } from "../controlPlaneView";
...
export function MissionsPage({ projectId }: { projectId: string }) {
  ...
  const missions = missionHistory(state, projectId);
  const control = latestControlPlane(state, projectId);
  const active = control?.mission ?? missions[0];
  ...
  const progress = selected ? Math.min(94, 30 + Math.min(run.cycleCount, 16) * 4) : 0;
  ...
  const inspector = <>
    ...
    <InspectorCard title="상태" meta={selected ? "활성" : "대기"}>
    ...
    <InspectorCard title="의존성" meta={`${openHumans.length} human`}>
      ...
      {!openHumans.length && <p className="product-muted">현재 사람 결정으로 막힌 범위가 없습니다.</p>}
    </InspectorCard>
  ...
  return <ProductWorkspace inspector={inspector}>
    <ProductHeader eyebrow="미션" title={selected?.objective ?? "현재 실행 중인 미션이 없습니다"} description={selected ? `${selected.role} · autonomy가 Intent와 Gap Graph에서 만든 실행 단위` : "새 gap이 우선순위 frontier에 들어오면 미션이 만들어집니다."} actions={selected && <StatusPill tone="working">RUNNING</StatusPill>} />

    <div className="mission-switcher">
      <div className="mission-switcher-label">미션 기록</div>
      {missions.length ? missions.slice(0, 6).map((mission, index) => <button className={`mission-switcher-item ${selected?.id === mission.id ? "active" : ""}`} key={mission.id} onClick={() => setSelectedKey(mission.id)}><span>{index === 0 ? "현재" : `이전 ${index}`}</span><strong>{mission.objective}</strong><small>{mission.role}</small></button>) : <div className="product-empty">미션 기록이 아직 없습니다.</div>}
    </div>

    {selected && <Surface className="mission-hero">
      <div className="mission-hero-top"><div><StatusPill tone="working">실행 중</StatusPill><span className="mission-id">{selected.id.slice(-10)}</span></div><span>{progress}%</span></div>
      <h2>{selected.objective}</h2>
      <p>{selected.role}</p>
      <div className="mission-progress"><span style={{ width: `${progress}%` }} /></div>
      <div className="mission-meta-row"><span>시작 신호 {relativeAge(selected.observedAt)} 전</span><span>cycle {run.cycleCount}</span><span>worker {project.settings.modelProvider ?? "auto"}</span></div>
    </Surface>}
    ...
    <div className="mission-detail-grid">
      <Surface>
        ...
        {tab === "의존성" && <div className="mission-simple-list">{openHumans.map((item) => <div key={item.id}><strong>{item.title}</strong><small>차단: {item.blockingScope.join(", ") || "없음"}</small></div>)}</div>}
