...
export function ControlCenterPage({ projectId }: { projectId: string }) {
  ...
  const openGapCount = (control?.counts.open ?? 0) + (control?.counts.investigating ?? 0) + (control?.counts.blocked ?? 0);
  const activeMissionCount = mission ? 1 : 0;
  ...
  const inspector = <>
    ...
    <InspectorCard title="현재 미션" meta={mission ? "실행 후보" : "없음"}>
      {mission ? <><p className="inspector-strong">{mission.objective}</p><p className="product-muted">역할 · {mission.role}</p><KeyValue label="진행 추정" value={`${missionProgress}%`} tone="working" /><KeyValue label="근거 조건" value={`${mission.evidenceContract.length}개`} /></> : <p className="product-muted">다음 discovery / prioritization cycle에서 미션이 선택됩니다.</p>}
    </InspectorCard>
  ...
  return <ProductWorkspace inspector={inspector}>
    ...
    <div className="metric-strip">
      <Metric label="열린 갭" value={openGapCount} hint={`미탐색 ${control?.counts.unexplored ?? 0}`} tone="warning" />
    ...
    <Surface className="current-mission-surface">
      <SurfaceHeader title="현재 미션" meta={mission ? `진행 ${missionProgress}%` : "미션 선택 대기"} action={mission && <button className="product-link-button" onClick={() => navigate(`${projectPath(projectId)}/missions`)}>상세 보기</button>} />
      {mission ? <div className="mission-focus">
        <div className="mission-focus-copy"><StatusPill tone="working">RUNNING</StatusPill><h3>{mission.objective}</h3><p>{mission.role} · 최신 autonomy 판단 {relativeAge(mission.observedAt)} 전</p></div>
    ...
    <div className="control-center-grid">
      <Surface>
        ...
        <div className="gap-list">{gaps.length ? gaps.slice(0, 5).map((gap) => <button className="gap-row" key={gap.id} onClick={() => navigate(`${projectPath(projectId)}/coverage`)}>
          ...
          <span className="gap-copy"><strong>{gap.title}</strong><small>{gap.category} · {gap.id}</small></span>
          <span className="gap-score">{gap.priority.toFixed(2)}</span>
        </button>) : <div className="product-empty">아직 priority frontier가 게시되지 않았습니다.</div>}</div>
      </Surface>
