...
export function NewProjectPage() {
  ...
  const maxModelCalls = 200;
  const [maxNativeTurns, setMaxNativeTurns] = useState(40);
  const maxNativeTokens = 250000;
  ...
  const onSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    const settings: Partial<ProjectSettings> = {
      budgetLimit: budget, maxHours, maxModelCalls, resourceLimitsDisabled: true, executionMode, maxNativeTurns, maxNativeTokens,
  ...
  const inspector = <>
    ...
    <InspectorCard title="리소스 한도" meta="자율 테스트">
      <KeyValue label="비용 hard stop" value="없음" tone="success" /><KeyValue label="토큰 hard stop" value="없음" tone="success" /><KeyValue label="모델 호출 상한" value="무제한" tone="success" /><KeyValue label="최대 시간" value={`${maxHours}h`} /><KeyValue label="재탐색" value="60m" />
    </InspectorCard>
