...
export function AppShell({ children }: PropsWithChildren) {
  ...
  return <div className={cn("app-frame app-frame-v3", isDesktopApp && "desktop-shell")}>
    <header className="ss-toolbar">
      ...
      <div className="ss-toolbar-right"><span className="ss-online"><i />{project?.status === "ACTIVE" ? "Worker 연결됨" : statusLabel(project?.status ?? "ACTIVE")}</span>{project && <span>{project.settings.resourceLimitsDisabled !== false ? `${project.budgetSpent.toFixed(2)} · 무제한` : `${project.budgetSpent.toFixed(2)} / ${project.settings.budgetLimit}`}</span>}<time>{clock.toLocaleTimeString("ko-KR", { hour: "2-digit", minute: "2-digit", hour12: false })}</time></div>
