...
export async function runNativeEpisode(store: CycleStateStore, projectId: string, options: NativeRuntimeOptions = {}): Promise<boolean> {
  ...
  await store.transact((state) => {
    ...
    if (!project.settings.resourceLimitsDisabled && native.accountedTokens >= limit(project.settings.maxNativeTokens, 250000, 10000000)) return stallProject(state, projectId, "Native 토큰 상한에 도달했습니다.");
    claimed = true;
    ...
    client.onNotification((method, params) => {
      ...
      if (method === "rawResponseItem/completed" && ["function_call", "function_call_output", "custom_tool_call", "custom_tool_call_output"].includes(String(item.type))) {
        ...
        await mutation((s) => {
          ...
          if (!initial.settings.resourceLimitsDisabled && tokens >= limit(initial.settings.maxNativeTokens, 250000, 10000000)) tokenLimited = true;
          return next;
        });
