...
function outputSchema(questions: Record<string, DecisionQuestion>): Record<string, unknown> {
  const properties = Object.fromEntries(Object.entries(questions).map(([key, question]) => {
    if (question.type === "noul") return [key, { type: "object", additionalProperties: false, required: ["kind", "probability"], properties: { kind: { type: "string", enum: ["noul"] }, probability: { type: "number", minimum: 0, maximum: 1 } } }];
    if (question.type === "choice") {
      const choices = Object.keys(question.criteria);
      return [key, { type: "object", additionalProperties: false, required: ["kind", "choice", "probabilities", "confidence"], properties: {
    ...
    return [key, { type: "object", additionalProperties: false, required: ["kind", "score", "probabilities", "confidence"], properties: {
...
export class CodexDecisionGateway implements DecisionGateway {
  ...
  async decide(request: DecisionBatchRequest): Promise<DecisionBatchResult> {
    ...
    const instruction = [
      ...
      "Probabilities are calibrated estimates, not permission grants. Hard policy and real-world verification are outside your authority.",
      "For choice questions, return a probability for every supplied option. For noul, probability means P(criteria is true).",
    ].join("\n");
    const result = await this.runner<{ answers: Record<string, DecisionAnswer> }>({ instruction, state: { purpose: request.purpose, state: request.state, questions: request.questions }, schema: outputSchema(request.questions), purpose: `decision-${request.purpose}`, signal: request.signal });
...
export class JevDecisionGateway implements DecisionGateway {
  ...
  constructor(options: JevGatewayOptions = {}) {
    ...
    this.endpoint = options.endpoint ?? process.env.TYPESAFE_API_URL?.trim() ?? "https://api.typesafe.ai/v1/systemone";
    this.model = options.model ?? process.env.TYPESAFE_JEV_MODEL?.trim() ?? "jev-latest";
    this.fetchImpl = options.fetchImpl ?? fetch;
    this.timeoutMs = Math.max(1_000, Math.min(120_000, options.timeoutMs ?? Number(process.env.TYPESAFE_TIMEOUT_MS ?? 20_000) || 20_000));
  }
  ...
  async decide(request: DecisionBatchRequest): Promise<DecisionBatchResult> {
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Jev API ${response.status}: ${(await response.text()).slice(0, 500)}`);
      ...
      for (const [key, question] of Object.entries(request.questions)) {
        ...
        const answer = item as Record<string, unknown>;
        if (question.type === "noul") normalized[key] = { kind: "noul", probability: clamp01(answer.noul) };
        else if (question.type === "choice") {
          ...
          const probabilities = Array.isArray(answer.probabilities) ? answer.probabilities.map(clamp01).slice(0, question.criteria.length) : [];
          while (probabilities.length < question.criteria.length) probabilities.push(0);
          normalized[key] = { kind: "score", score: typeof answer.score === "number" ? answer.score : 0, probabilities, confidence: clamp01(answer.confidence) };
        }
      }
      const usageRaw = value.usage && typeof value.usage === "object" ? value.usage as Record<string, unknown> : {};
      const inputTokens = Math.max(0, Number(usageRaw.input_tokens ?? usageRaw.inputTokens ?? usageRaw.tokens ?? 0) || 0);
      const price = Math.max(0, Number(process.env.JEV_COST_PER_MILLION ?? 0.042) || 0.042);
      const usage: ModelUsage = { modelVersion: `jev:${String(value.model ?? this.model)}`, tokens: inputTokens, inputTokens, outputTokens: 0, usageKnown: inputTokens > 0, cost: Number(((inputTokens / 1_000_000) * price).toFixed(6)), latencyMs: Date.now() - startedAt };
...
export class HybridDecisionGateway implements DecisionGateway {
  ...
  async decide(request: DecisionBatchRequest): Promise<DecisionBatchResult> {
    if (this.jev.isConfigured()) {
      ...
      catch { /* bounded judgment may safely fall back to the configured Codex decision adapter */ }
    }
...
export function decisionProviderStatus(): DecisionProviderStatus {
  ...
  if (requested === "jev") return { requested, effective: configured ? "jev" : "unavailable", jevConfigured: configured, jevModel: process.env.TYPESAFE_JEV_MODEL?.trim() || "jev-latest", detail: configured ? "Jev bounded decision layer is configured." : "Set TYPESAFE_API_KEY to enable Jev." };
  if (requested === "hybrid") return { requested, effective: configured ? "hybrid" : "codex-cli", jevConfigured: configured, jevModel: process.env.TYPESAFE_JEV_MODEL?.trim() || "jev-latest", detail: configured ? "Jev is primary and Codex is the decision fallback." : "Jev is not configured; Codex decision fallback is active." };
  return { requested, effective: "codex-cli", jevConfigured: configured, jevModel: process.env.TYPESAFE_JEV_MODEL?.trim() || "jev-latest", detail: "Codex CLI handles bounded decisions. Set SAKASAKA_DECISION_PROVIDER=jev or hybrid after Jev access is approved." };
}
